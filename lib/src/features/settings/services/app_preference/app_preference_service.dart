...
class AppPreferenceService {
  ...
  Future<void> _initAllPreferences() async {
    // init layout preference
    final layoutPref = _prefs.getString(PreferenceKey.layout.value);
    if (layoutPref == null) {
      final couldSet = await _prefs.setString(
        PreferenceKey.layout.value,
  ...
  dynamic getPreference(PreferenceKey key) {
    try {
      ...
      if (value == null) {
        ...
        log('Retrieved preference value: ${value.toString()} for key: ${key.value}');
        return value;
      }
    } catch (e) {
      throw CouldNotGetPreferenceException();
